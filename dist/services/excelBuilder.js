...
exports.buildExcelBuffer = buildExcelBuffer;
...
async function buildExcelBuffer(boqData, formData, criticalFacts) {
...
// Summary Sheet
// ─────────────────────────────────────────────────────────────────────────────
function buildSummarySheet(wb, boqData, formData, cf) {
    ...
    if (cf.roofSlope)
        addRow('Roof Slope', cf.roofSlope, true);
    if (cf.roofSheetSpec)
        addRow('Roof Sheet', cf.roofSheetSpec, true);
    if (cf.roofInsulationSpec)
        addRow('Insulation', cf.roofInsulationSpec, true);
    if (cf.wallCladdingSpec)
        addRow('Wall Cladding', cf.wallCladdingSpec, true);
    if (cf.structuralSystem)
        addRow('Structural System', cf.structuralSystem, true);
...
// BOQ Sheet
// ─────────────────────────────────────────────────────────────────────────────
function buildBOQSheet(wb, boqData) {
...
// ─────────────────────────────────────────────────────────────────────────────
function buildGenericSheet(wb, name, rows, headers) {
    ...
    let alt = false;
    rows.forEach(row => {
        if (!Array.isArray(row) || row[0] === headers[0])
