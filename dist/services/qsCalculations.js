...
exports.validateCriticalFacts = validateCriticalFacts;
exports.calculateGeometricQuantities = calculateGeometricQuantities;
exports.extractCraneData = extractCraneData;
exports.identifyBuildingPackages = identifyBuildingPackages;
// ── Safe integer helper ───────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
function validateCriticalFacts(drawingFacts, formData) {
    const df = drawingFacts || {};
    const roofSlope = df.roofSlope && df.roofSlope !== 'NOT SHOWN' ? df.roofSlope.trim() : null;
    const roofSheetSpec = df.roofSheetSpec && df.roofSheetSpec !== 'NOT SHOWN' ? df.roofSheetSpec.trim() : null;
    const roofInsulationSpec = df.roofInsulationSpec && df.roofInsulationSpec !== 'NOT SHOWN'
        ? df.roofInsulationSpec.trim()
        : null;
    const wallSpec = df.wallCladdingSpec && df.wallCladdingSpec !== 'NOT SHOWN'
        ? df.wallCladdingSpec.trim()
        : 'Brick wall 1m height base + Metal sheet wall above to eave';
    const structuralSystem = df.structuralSystem && df.structuralSystem !== 'NOT SHOWN'
    ...
    console.log(`[QS] criticalFacts: type=${df.projectType || 'unknown'} ` +
...
// ─────────────────────────────────────────────────────────────────────────────
function calculateGeometricQuantities(drawingFacts, userArea, formData) {
    let factoryLength = 0;
    let factoryWidth = 0;
    ...
    console.log(`[QS] geoQtys: ${gq.factoryLength}x${gq.factoryWidth}m ` +
...
// extractCraneData
// ─────────────────────────────────────────────────────────────────────────────
function extractCraneData(drawingFacts) {
    const df = drawingFacts || {};
    const craneDescriptions = [];
    const sources = [
    ...
    if (craneDescriptions.length === 0) {
        return {
            found: false,
            promptNote: 'No crane data confirmed from drawing. For a factory project, include a crane allowance ' +
    ...
    return {
        ...
        promptNote: 'The following crane items are confirmed or likely visible on the drawing. ' +
...
// ─────────────────────────────────────────────────────────────────────────────
function identifyBuildingPackages(drawingFacts, userArea) {
    const df = drawingFacts || {};
    const packages = [];
    // Try from workPackages extraction
    if (Array.isArray(df.workPackages) && df.workPackages.length > 1) {
        df.workPackages.forEach(row => {
            if (!Array.isArray(row) || String(row[0]) === 'Package')
                return;
            const name = String(row[0] || '').trim();
            if (name)
                packages.push({ name, scopeNote: String(row[1] || ''), confidence: String(row[2] || '') });
        });
    }
    ...
    return {
        ...
        totalArea,
        summary: packages
            .map(p => `• ${p.name}: ${p.area ? p.area + ' m²' : 'area TBC'}` +
