...
import { geminiText, parseGeminiJson } from './gemini.js';
import { extractDrawingFacts } from './qsExtraction.js';
import {
  validateCriticalFacts,
  calculateGeometricQuantities,
  extractCraneData,
  identifyBuildingPackages
} from './qsCalculations.js';
import { buildPromptA, buildPromptB } from '../prompts/boqPrompts.js';
import { applyCalculatedQuantities, ensureRequiredDivisions } from './quantityInjector.js';
import { buildExcelBuffer } from './excelBuilder.js';
import { FormData, BOQData, QSGenerationResult, ConfirmedFacts, DrawingFacts } from '../types/index.js';
import { v4 as uuidv4 } from 'uuid';

export async function generateQS(
  formData: FormData,
  drawingBase64?: string,
  drawingMimeType?: string,
  confirmedFacts?: ConfirmedFacts
): Promise<QSGenerationResult> {
  const jobId = uuidv4();
  ...
  try {
    ...
    if (confirmedFacts && (confirmedFacts.roofSlope || confirmedFacts.roofSheetSpec || confirmedFacts.factoryFootprint)) {
      ...
      drawingFacts = await extractDrawingFacts(formData, drawingBase64, drawingMimeType);
    }

    // ── Step 2: Calculate derived values ─────────────────────────────────────
    const criticalFacts   = validateCriticalFacts(drawingFacts, formData);
    const geoQtys         = calculateGeometricQuantities(drawingFacts, formData.totalArea, formData);
    const craneData       = extractCraneData(drawingFacts);
    const buildingPackages = identifyBuildingPackages(drawingFacts, formData.totalArea);
    ...
    const [rawA, rawB] = await Promise.all([
      geminiText(buildPromptA(formData, criticalFacts, geoQtys, craneData, buildingPackages), 32768),
      geminiText(buildPromptB(formData, criticalFacts, geoQtys, craneData, buildingPackages,
        { totalBase: 0, totalLow: 0, totalHigh: 0 }), 32768)
    ]);

    // ── Step 4: Parse ─────────────────────────────────────────────────────────
    const boqDataA = parseGeminiJson<BOQData>(rawA, 'BOQ Part A') || {};
    const boqDataB = parseGeminiJson<BOQData>(rawB, 'BOQ Part B') || {};
    ...
    boqData.boqItems?.push(
      ['', '', '', '', '', '', '', '', '', '', 'Section', '', ''] as any,
      ['P1', '', `Preliminaries (${formData.prelimPct||10}%)`, 'lot', 1, '', Math.round(prelims*0.8), prelims, Math.round(prelims*1.2), 'QS Parameter', 'Estimated', 'Low', ''] as any,
      ['P2', '', `Contingency (${formData.contingencyPct||15}%)`, 'lot', 1, '', Math.round(contingency*0.9), contingency, Math.round(contingency*1.1), 'QS Parameter', 'Estimated', 'Low', ''] as any,
      ['P3', '', `Profit / Markup (${formData.profitPct||8}%)`, 'lot', 1, '', Math.round(profit*0.9), profit, Math.round(profit*1.1), 'QS Parameter', 'Confirmed', 'High', ''] as any,
      ['P4', '', `VAT (${formData.vatPct||10}%)`, 'lot', 1, '', Math.round(vat*0.9), vat, Math.round(vat*1.1), 'QS Parameter', 'Confirmed', 'High', ''] as any,
      ['TOTAL', '', 'GRAND TOTAL (incl. VAT) — Class D ±30–50%', '', '', '', Math.round(totalBase*0.75), totalBase, Math.round(totalBase*1.35), '', 'Summary', '', ''] as any
    );

    // ── Step 7: Post-processing ───────────────────────────────────────────────
    boqData = applyCalculatedQuantities(boqData, geoQtys);
    boqData = ensureRequiredDivisions(boqData, geoQtys, craneData);

    console.log(`[QS] Final: ${boqData.boqItems?.length || 0} rows`);

    // ── Step 8: Excel ─────────────────────────────────────────────────────────
    const excelBuffer = await buildExcelBuffer(boqData, formData, criticalFacts);
...
function extractSubtotal(data: BOQData): number {
  if (!Array.isArray(data.boqItems)) return 0;
  ...
  return data.boqItems.reduce((sum, row) => {
    ...
    if (status === 'section' || status === 'summary') return sum;
    const val = Number(row[7]);
