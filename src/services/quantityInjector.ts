...
import { BOQData, BOQRow, GeoQuantities, CraneData } from '../types/index.js';
...
// ─────────────────────────────────────────────────────────────────────────────
export function applyCalculatedQuantities(
  boqData: BOQData,
  geoQtys: GeoQuantities
): BOQData {
  if (!boqData.boqItems?.length) return boqData;
  ...
  // keyword → qty + source
  const rules: { match: string; qty: number; unit?: string; source: string }[] = [
  ...
  boqData.boqItems = boqData.boqItems.map(row => {
    if (!Array.isArray(row) || row.length < 13) return row;
    const desc = String(row[2] || '').toLowerCase();
    const status = String(row[10] || '').toLowerCase();
    if (status === 'section') return row;
    ...
    for (const rule of rules) {
      if (desc.indexOf(rule.match) === -1) continue;
      if (rule.unit && row[3] && String(row[3]).trim() !== rule.unit) continue;

      const amounts = computeAmounts(String(row[5] || ''), rule.qty);
      const newRow = [...row] as BOQRow;
      newRow[4]  = rule.qty;
...
// ─────────────────────────────────────────────────────────────────────────────
export function ensureRequiredDivisions(
  boqData: BOQData,
  geoQtys: GeoQuantities,
  craneData: CraneData
): BOQData {
  if (!boqData.boqItems) boqData.boqItems = [];
  ...
  function hasDiv(code: string): boolean {
    return items.some(r =>
      Array.isArray(r) &&
  ...
  boqData.boqItems = items.filter(r => {
    ...
    return !toRemove.some(code =>
      sec.startsWith(code) || desc.includes(`DIVISION ${code}`)
    );
  });

  // Division 23 — Cranes
  if (!hasDiv('23') && !hasDiv('2300')) {
    boqData.boqItems.push(
      ['', '', '', '', '', '', '', '', '', '', 'Section', '', ''] as unknown as BOQRow,
      ['A23', '2300', 'DIVISION 23: BUILDING EQUIPMENT (CRANES)', '', '', '', '', '', '', '', 'Section', '', ''] as unknown as BOQRow,
      ['23.1', '2306', 'Single Girder Bridge Crane (1.5T, span ~15.5m, hoist 6m)', 'set', 3, '350,000,000–700,000,000', 1050000000, 1575000000, 2100000000, 'Drawing — 3 bays', 'Specialist Quotation Required', 'Low', '14-20 week lead time'] as unknown as BOQRow,
      ['23.2', '2307', 'Double Girder Bridge Crane (5T, span ~19.2m, hoist 10m)', 'set', 1, '800,000,000–1,500,000,000', 800000000, 1150000000, 1500000000, 'Drawing — 1 bay', 'Specialist Quotation Required', 'Low', 'Confirm with vendor'] as unknown as BOQRow,
      ['23.3', '2308', 'Crane Runway Beams, Rails and End Stops', 'lot', 1, '500,000,000–900,000,000', 500000000, 700000000, 900000000, 'QS Allowance', 'Estimated', 'Low', ''] as unknown as BOQRow,
      ['23.4', '2309', 'Crane Load Testing, Commissioning and Certificates', 'lot', 1, '50,000,000–120,000,000', 50000000, 85000000, 120000000, 'QS Allowance', 'Estimated', 'Low', ''] as unknown as BOQRow
    );
  ...
  boqData.boqItems.push(
    ['', '', '', '', '', '', '', '', '', '', 'Section', '', ''] as unknown as BOQRow,
    ['A33', '3300', 'DIVISION 33: LANDSCAPING WORK', '', '', '', '', '', '', '', 'Section', '', ''] as unknown as BOQRow,
    ['33.1', '3301', 'Topsoil Preparation and Fertiliser', 'm²', greenArea, '30,000–80,000', Math.round(greenArea * 30000), Math.round(greenArea * 55000), Math.round(greenArea * 80000), 'Drawing ratio board — 3,559m²', 'Estimated', 'Medium', ''] as unknown as BOQRow,
    ['33.2', '3302', 'Grass Turf (Cỏ Nhật or equivalent)', 'm²', greenArea, '20,000–50,000', Math.round(greenArea * 20000), Math.round(greenArea * 35000), Math.round(greenArea * 50000), 'Drawing ratio board — 3,559m²', 'Estimated', 'Medium', ''] as unknown as BOQRow,
    ['33.3', '3303', 'Trees and Ornamental Planting', 'lot', 1, '30,000,000–80,000,000', 30000000, 55000000, 80000000, 'QS Allowance', 'Estimated', 'Low', ''] as unknown as BOQRow
  );

  // Division 34 — Sub-buildings
  boqData.boqItems.push(
    ['', '', '', '', '', '', '', '', '', '', 'Section', '', ''] as unknown as BOQRow,
    ['A34', '3400', 'DIVISION 34: SUB-BUILDING WORKS', '', '', '', '', '', '', '', 'Section', '', ''] as unknown as BOQRow,
    ['34.1', '3411', 'Utility Block — Tech + Pump + Compressor + Electrical + Transformer Rooms (5m×25m)', 'lot', 1, '500,000,000–1,200,000,000', 500000000, 850000000, 1200000000, 'Drawing A-301', 'Estimated', 'Medium', ''] as unknown as BOQRow,
    ['34.2', '3411b', 'Scrap Room + Hazardous Waste Room (5m×16m)', 'lot', 1, '80,000,000–180,000,000', 80000000, 130000000, 180000000, 'Drawing A-401', 'Estimated', 'Medium', ''] as unknown as BOQRow,
    ['34.3', '3411d', 'Car Parking + Bike Parking Shelter (6m×23m)', 'lot', 1, '150,000,000–350,000,000', 150000000, 250000000, 350000000, 'Drawing A-501', 'Estimated', 'Medium', ''] as unknown as BOQRow,
    ['34.4', '3412', 'LPG Room / Gas Storage Enclosure (3m×10m)', 'lot', 1, '80,000,000–160,000,000', 80000000, 120000000, 160000000, 'Drawing A-601', 'Estimated', 'Medium', ''] as unknown as BOQRow,
    ['34.5', '3402', 'Flag Pole (Stainless Steel, H≈13m)', 'set', 1, '25,000,000–60,000,000', 25000000, 42500000, 60000000, 'Drawing A-901', 'Estimated', 'Medium', ''] as unknown as BOQRow,
    ['34.6', '3403e', 'Septic Tanks (factory + guardhouse)', 'lot', 2, '20,000,000–50,000,000', 40000000, 70000000, 100000000, 'QS Allowance', 'Estimated', 'Low', ''] as unknown as BOQRow
  );

  // Division 35 — Signboards
  boqData.boqItems.push(
    ['', '', '', '', '', '', '', '', '', '', 'Section', '', ''] as unknown as BOQRow,
    ['A35', '3500', 'DIVISION 35: SIGNBOARD AND BRANDING WORK', '', '', '', '', '', '', '', 'Section', '', ''] as unknown as BOQRow,
    ['35.1', '3502', 'Company Name Signboard at Main Gate (L≈14.8m × H≈2.0m)', 'set', 1, '30,000,000–80,000,000', 30000000, 55000000, 80000000, 'Drawing A-1001', 'Estimated', 'Low', ''] as unknown as BOQRow,
    ['35.2', '3503', 'Company Name Signboard at Side Gate (L≈8.7m × H≈2.0m)', 'set', 1, '20,000,000–50,000,000', 20000000, 35000000, 50000000, 'Drawing A-1001', 'Estimated', 'Low', ''] as unknown as BOQRow,
    ['35.3', '3539', 'Factory Logo / Company Name on Facade (Stainless Steel)', 'set', 1, '15,000,000–50,000,000', 15000000, 32500000, 50000000, 'Drawing elevation', 'Estimated', 'Low', ''] as unknown as BOQRow
  );
